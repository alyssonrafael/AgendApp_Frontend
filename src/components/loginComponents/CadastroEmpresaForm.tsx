...
export default function CadastroEmpresaForm() {
  ...
  const handlePress = async (data: CadastroFormData) => {
    ...
    try {
      ...
      showMessage(
        "success",
        "Cadastro bem sucedido, Redirecionando para o login!"
      );
      setTimeout(() => {
      ...
      if (error.response) {
        ...
        switch (status) {
          case 400:
            showMessage(
              "alert",
              "Verifique os dados e tente novamente! Talvez seu email já esteja em uso!"
            );
            break;
          case 409:
            showMessage(
              "alert",
              "Já existe empresa com esse nome! Verifique e tente novamente."
            );
            break;
          case 500:
            showMessage(
